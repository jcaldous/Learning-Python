...
def pushNotebook(name, uuid,
    ...
    for i in range(10):
        try:
            ...
            print('Submission failed: are you online? Make sure you have wireless on')
            break
...
def check_figure(tag, given, ev, extra):
    ...
    if nbars:
        BarWeight = 1.0
        gbars = given['bars'].shape[1]
        BOK = np.zeros(nbars)
        for i in range(gbars):
            gbar = given['bars'][:,i]
            for j in range(nbars):
                if not BOK[j]:
                    ebar = ev['bars'][:,j]
                    BOK[j] = np.allclose(ebar, gbar, rtol=rtol, atol=atol)
                    if BOK[j]:
                        break
        BarScore = np.mean(BOK)

        if BarScore == 0:
            print(tag, 'bar values incorrect')
        elif BarScore < 1.0:
            print(tag, 'some bar values incorrect')
    else:
        BarScore = 0
...
try:
    ...
    def test_online():
        ...
        if expected.get('_monitor', False):
            url = 'http://152.2.128.56/Courses/comp116-001-f16/media/monitor.txt?%s' % getpass.getuser()
